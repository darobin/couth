...
angular.module("CouthClient", ["CouthResourceAPI"])
    ...
    .directive("couthRef", function () {
        return function (scope, element, attrs) {
            ...
            ,   def = attrs["default"]
            ;
            console.log(target, source, show, value, def);
            // XXX
            //  FUCK THIS!
            //  just use a drop down
            //  we populate it
            //  it's directly integrated
            //
            //  also, when exposeUsers is on, a form should be generated
            //  and it should be possible to provide extensions to its basic type (that only the form takes
            //  into account)
            //  that way you can edit users properly
        };
        
    })
    .controller("CouthCtrl", function ($scope, $rootScope, $http, $document) {
        ...
        $document.bind("keypress", function (evt) {
            if (evt.keyCode === 27) {
                $scope.$couthSuccess = $scope.$couthError = false;
                $scope.$digest();
            }
        });
        ...
        $scope.$on("couth:edit", function (evt, obj) {
            ...
            if (!ed) return;
            ed.$couthOriginal = obj;
            ed.$couthInstance = angular.fromJson(angular.toJson(obj));
            ed.$couthMode = "edit";
