/*global provides send getRow log start*/
...
function makeList (perm) {
    return "function (head, req) {\n" +
           "    return require('lib/utils').listFor(head, req, '" + perm + "', provides, send, getRow, log);\n" +
           "}";
}
...
function makeRead () {
    return function (head, req) {
                return require("lib/utils").showFor(head, req, provides, send, getRow, start, log);
           }.toString();
}
