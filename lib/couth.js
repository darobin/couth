...
,   buildResourceAPI:   function () {
        ...
        for (var t in this.types) {
            couthTypes.push(t);
            type2key[t] = this.types[t].crudOptions.id;
        }
        // XXX here we need to build something that shares the list of types and crudOptions
        //     but doesn't generate a script. This is then used by the API provider

        // this.addRewrite(path, path);
        // this.addAttachment( path.replace(/^\//, "")
        //                 ,   new Buffer(apiTmpl({ couthTypes: JSON.stringify(couthTypes), type2key: JSON.stringify(type2key) }))
        //                 ,   "text/javascript");
    }
};
