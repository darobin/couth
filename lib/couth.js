...
,   buildResourceAPI:   function () {
        ...
        function inFunc (key, val) {
            if (_.isFunction(val)) return val.toString();
            return val;
        }
        var js = "angular.module('CouthResourceAPI', ['ngResource'])\n";
        for (var t in this.types) {
            var data = this.types[t].resourceAPI();
            js += "    .factory('" + data.name + "', function ($resource) {\n" +
                  "        return $resource('" + data.path + "', " + JSON.stringify(data.paramDefaults) + ",\n" +
                  JSON.stringify(data.actions, inFunc, 4) + "\n" +
                  "        );\n    })\n"
            ;
        }
        js += ";\n";
        var path = "/couth/js/resource-api.js";
        this.addRewrite(path, path);
