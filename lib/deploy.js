...
Couth.prototype.deploy = function (done) {
    ...
    this.loadCurrentDesign(function (err) {
        ...
        if (conf.webAppRoot) {
            var root, index;
            if (_.isString(conf.webAppRoot)) {
                root = conf.webAppRoot;
                index = conf.index || "/";
            }
            else {
                root = conf.webAppRoot.base;
                index = conf.webAppRoot.index || conf.index || "/";
            }
            self.addRewrite(root, index);
            if (!/\*$/.test(root)) {
                if (!/\/$/.test(root)) root += "/";
                root += "*";
            }
...
Couth.prototype.loadCurrentDesign = function (cb) {
    var self = this;
    function loadDoc () {
        self.cradle.get("_design/couth", function (err, res) {
            if (err) {
